...
| 层 | 选型 | 理由 |
|---|---|---|
| ASR 推理 | **sherpa-onnx Android AAR**（C++ 内核） | 端侧、离线、粤语 zipformer 模型、官方 Android 支持 |
...
1. `./gradlew testDebugUnitTest lintDebug` 全绿
2. 正字引擎单测覆盖：粤拼解析、词典加载、正字化（"无该"→"唔該"）、候选排序
3. `./gradlew assembleDebug` 产出 APK，可安装
4. IME 可启用：设置页 → 启用 Roar 键盘 → 任意输入框调出 → 语音按钮录音 → 候选栏出现 → 点选上屏
5. 真机验证（用户执行）：华为/其他 Android 手机录音转正字端到端
