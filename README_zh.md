...
- [x] **IME Compose 宿主修复（2026-09-09）**：服务自供 `ViewTreeLifecycleOwner` +
      `ViewTreeSavedStateRegistryOwner`（compose-ui 1.7 在 IME 窗口硬性要求两者，
      修复前键盘在真机首弹即崩）
- [x] **默认深色模式（2026-09-09）**：`RoarTheme`（API 31+ 动态深色 / 低版本静态深色回落）
      + `Theme.Roar`（Material3 Dark 父主题）——应用无视系统明暗设置，恒为深色（产品决定）
- [x] 下载完整性加固（2026-09-09）：体积 + SHA-256 校验下沉到逐源循环（官方源下载
      损坏自动切镜像），新增进程级下载互斥锁（杜绝两线程写同一 .part 互相破坏）
- [ ] 真机端到端验证（见下方清单）
